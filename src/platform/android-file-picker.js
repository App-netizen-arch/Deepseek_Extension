/**
 * JavaScript-side file-picker protocol adapter retained for upstream UI
 * compatibility. The native Android/Kotlin application is not part of this
 * repository; this module only speaks the existing window event protocol.
 */

export function isNativeFilePickerAvailable() {
  return typeof window !== "undefined" && window.AndroidBridge != null && typeof window.AndroidBridge.pickFiles === "function";
}
...
export function nativePickFiles(mode = "files") {
  return new Promise((resolve, reject) => {
    ...
    const clearLaunchTimer = () => { clearTimer(launchTimer); launchTimer = null; };
    const clearReturnGraceTimer = () => { clearTimer(returnGraceTimer); returnGraceTimer = null; };
    const clearReadStallTimer = () => { clearTimer(readStallTimer); readStallTimer = null; };
    ...
    const rejectWithCode = (code) => settle(() => reject(new Error(code)));

    const parseCompletePayload = () => {
      let payload;
      try { payload = JSON.parse(chunks.join("")); } catch { rejectWithCode(PICK_ERRORS.MALFORMED); return; }
      if (!payload || typeof payload !== "object" || Array.isArray(payload)) { rejectWithCode(PICK_ERRORS.MALFORMED); return; }
      if (payload.error === "cancelled") { settle(() => resolve({ files: [], cancelled: true })); return; }
      if (payload.error) { settle(() => reject(new Error(String(payload.error)))); return; }
      settle(() => resolve({
        ...payload,
    ...
    const handler = (event) => {
      ...
      clearLaunchTimer();
      clearReturnGraceTimer();
      if (!detail || detail.v !== 2 || typeof detail.kind !== "string") { rejectWithCode(PICK_ERRORS.MALFORMED); return; }
      if (detail.kind === "status") {
        if (detail.phase !== "opened" && detail.phase !== "reading") { rejectWithCode(PICK_ERRORS.MALFORMED); return; }
        if (detail.phase === "reading") armReadStallTimer();
        return;
      }
      if (detail.kind !== "chunk") { rejectWithCode(PICK_ERRORS.MALFORMED); return; }
      if (!Number.isInteger(detail.seq) || !Number.isInteger(detail.total) || detail.total <= 0 || detail.seq < 0 || detail.seq >= detail.total || typeof detail.data !== "string") { rejectWithCode(PICK_ERRORS.MALFORMED); return; }
      if (expectedTotal === null) expectedTotal = detail.total;
      else if (expectedTotal !== detail.total) { rejectWithCode(PICK_ERRORS.MALFORMED); return; }
      armReadStallTimer();
      if (chunks[detail.seq] === undefined) { chunks[detail.seq] = detail.data; receivedChunks += 1; }
      if (receivedChunks === expectedTotal) parseCompletePayload();
    };

    const visibilityHandler = () => {
      if (settled || typeof document === "undefined") return;
      if (document.visibilityState === "hidden") { hiddenSinceLastVisible = true; clearReturnGraceTimer(); return; }
      if (document.visibilityState === "visible" && hiddenSinceLastVisible) { hiddenSinceLastVisible = false; armReturnGraceTimer(); }
    };
...
export function pickedEntryToFile(entry) {
  ...
  if (entry?.encoding === "base64") {
    ...
    return new File([bytes], name, { type: entry.mime || "application/octet-stream" });
  }
...
export function buildFolderFileFromNative(files, folderName) {
  ...
  let content = "Directory Tree:\n" + renderPathTree(tree) + "\n\n========================================\n\n";
  for (const file of textFiles) content += `\n\n--- [FILE: ${file.name}] ---\n\n${file.content}`;
