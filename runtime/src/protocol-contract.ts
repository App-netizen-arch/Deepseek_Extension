...
export function makeRequest<T>(
  ...
  type: BdsRequestKind,
  payload?: T,
  context?: Pick<BdsRequestEnvelope, "sessionId" | "projectId">,
): BdsRequestEnvelope<T> {
  return {
    version: BDS_PROTOCOL_VERSION,
    id,
    type,
    timestamp: Date.now(),
...
export function makeResponse<T>(requestId: string, type: BdsEventKind, ok: boolean, value?: T | { code: string; message: string }): BdsResponseEnvelope<T> {
  if (ok) {
    return {
      ...
      inReplyTo: requestId,
      ok: true,
      ...(value === undefined ? {} : { payload: value as T }),
    };
  }
  return {
    version: BDS_PROTOCOL_VERSION,
    ...
    inReplyTo: requestId,
    ok: false,
    error: value as { code: string; message: string },
  };
}
