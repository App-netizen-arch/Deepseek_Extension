...
export function makeResponse<T>(requestId: string, type: BdsEventKind, ok: boolean, value?: T | { code: string; message: string }): BdsResponseEnvelope<T> { if (ok) return { version: BDS_PROTOCOL_VERSION, id: crypto.randomUUID(), type, timestamp: Date.now(), inReplyTo: requestId, ok: true, ...(value === undefined ? {} : { payload: value as T }) }; return { version: BDS_PROTOCOL_VERSION, id: crypto.randomUUID(), type, timestamp: Date.now(), inReplyTo: requestId, ok: false, error: value as { code: string; message: string } }; }
export function validateRequestEnvelope(value: unknown): asserts value is BdsRequestEnvelope { if (!value || typeof value !== "object") throw new Error("invalid protocol envelope"); const v = value as Record<string, unknown>; if (v.version !== BDS_PROTOCOL_VERSION) throw new Error("unsupported BDS protocol version"); if (typeof v.id !== "string" || v.id.length < 1 || v.id.length > 128) throw new Error("invalid request id"); if (typeof v.type !== "string") throw new Error("request type is required"); if (!Number.isSafeInteger(v.timestamp)) throw new Error("invalid request timestamp"); if (v.sessionId !== undefined && (typeof v.sessionId !== "string" || v.sessionId.length > 128)) throw new Error("invalid sessionId"); if (v.projectId !== undefined && (typeof v.projectId !== "string" || v.projectId.length > 128)) throw new Error("invalid projectId"); }
export const CANONICAL_BDS_TAGS = Object.freeze(["AGENT_STATUS","AGENT_CONTROL","LOCAL_EXEC","WEB_AGENT","MATH_ANALYZE","MATH_PDF","MATH_ASK","TIKZ_RENDER","CODE_AGENT","AGENT_LOGIN","MEMORY_WRITE","MEMORY_READ","MEMORY_LIST","MEMORY_DELETE","PROJECT_CONTEXT","AGENT_DEFINE","SUBAGENT","MCP_MANAGE","WORKFLOW","SKILL_LOAD","PERMISSION_SET","CONFIG_SET","SESSION_EXPORT","VISUALIZER","COWORK","CODE_PANEL","CHAT_CLEAN"] as const);
