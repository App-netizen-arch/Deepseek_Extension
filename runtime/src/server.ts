...
export function createRuntimeServer() {
  ...
  server.on("upgrade", (req, socket, head) => { if (!authenticateBearer(req.headers.authorization, TOKEN)) { socket.destroy(); return; } wsServer.handleUpgrade(req, socket, head, (ws) => wsServer.emit("connection", ws, req)); });
  return server;
}

export const runtimeServer = createRuntimeServer();
